    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch("/api/main/register", {
                ...
                body: JSON.stringify({ nis, name, password, kelas }),
            })
            ...
            if (res.ok) {
                ...
                setTimeout(() => {
                    window.location.href = "/AuthenticationPage/login"
                }, 1500)
            } else {
                setFormError(data.error || "Gagal register")
            }
        } catch (err: unknown) {
            const message =
                err instanceof Error ? err.message : "Terjadi kesalahan saat register"
